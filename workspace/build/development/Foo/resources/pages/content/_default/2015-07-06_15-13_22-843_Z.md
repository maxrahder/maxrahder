...
We'll update Yelp Extplorer to use route information to save and restore the selected tab,
...
The reason we're using the exclaimation point in the pattern is to comply with Google's recommendation for
...
Save and refresh. Test this by manually adding `#!map` to the URL 
(i.e. <a href="http://localhost:1841/SenchaTraining/ExtJS_5/labs/yelpextplorer/index.html#!map" target="lab">http://localhost:1841/SenchaTraining/ExtJS_5/labs/yelpextplorer/index.html#!map</a>). 
...
Edit the tab panel view, and assign an +itemId+ to each item. Set the IDs to *'map'*, *'grid'*, *'view'*,
and *'chart'*. 
...
From now on, the routes logic will be responsible for setting a default city and
category, so edit `app/view/main/MainModelShared.js` and remove the entries for *city* and *category*.

Then edit `app/view/main/Routes.js` and replace its contents with this code:

Replace the +processRoute()+ with this code. 
...
You also want to update the route as the user changes the city. For that you'll need to procedurally bind to *{city}* and 
update the route accordingly.

Edit `app/view/main/MainController.js` and in +initViewModel()+ method and add this statement

    vm.bind('{city}', me.updateHash, me);

The +updateHash+ method you started using in the previous lab step already uses the view 
model *{city}*, so the new binding should work right away. Save and refresh, and when you change
the city you'll see it in the URL.
...
This is just like what you had before, but it adds the code to process the category.
...
You may have noticed that as you changed city or category, then pressed Back, there is a one
second delay before the businesses are fetched. That's because of the buffered calls set up 
in the controller. Waiting a second makes sense *as the user types*, but it doesn't make sense
when pressing Back and the city or category is changed in its entirity. 

To fix this you need view model properties that hold the values the user types &mdash; these
are processed separately from the city and category used for the Yelp query.

First, edit `classic/src/view/businesses/Filter.js` and change the bindings from *city* an *category* 
to *typedCity* and *typedCategory*. (If you did the optional Publishes lab, these bindings are in 
...
Now edit `app/view/main/MainController.js` and replace +initViewModel+ with this code:
    ...
    initViewModel: function(vm) {
        ...
        vm.bind('{location}', me.loadBusinesses, me);
...
This code has *typedCity* and *typedCategory* update *city* and *category*, after waiting a second.
The code also puts all city-related and category-related processes in their own methods &mdash; it's
cleaner that way.

Save and refresh, and everything should work, and in addition, changing city or category, then pressing
Back, should no longer have the delay before centering the map and fetching businesses. 
