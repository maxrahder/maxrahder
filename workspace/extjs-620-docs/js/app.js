...
(function() {
    ...
    function runFiddleExample (wrap) {
        ...
        if (myToolkit === 'modern') {
            data.codes.codes[0].code = data.codes.codes[0].code.replace(/(renderTo\s*:\s*(?:Ext\.getBody\(\)|document\.body))/, 'fullscreen: true');
        }

        if (pageName.toLowerCase().indexOf('ux') > -1) {
            data.codes.codes = data.codes.codes.concat([{
                type : 'js',
                name : 'ux-build.js',
    ...
    function showMemberTypeMenu (e) {
        ...
        ExtL.each(memberList, function (item) {
            ...
            // hidden
            // if (item.offsetHeight && item.id.indexOf('placeholder') !== 0) { // commenting per DOCS-239
            if (item.offsetHeight) { // commenting per DOCS-239
